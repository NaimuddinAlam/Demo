...
class EquipmentAdapter(val constants: Context, val list: List<AllData>? = null, val   createBy1: UpdateComplaintStatus):
    RecyclerView.Adapter<EquipmentAdapter.Myholder>() {


    var item_list: ArrayList<ColorSpace.Model>? = null
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): EquipmentAdapter.Myholder {
        return(Myholder(LayoutInflater.from(constants).inflate(R.layout.cardcomp, null)))
    }


    @RequiresApi(Build.VERSION_CODES.O)
    override fun onBindViewHolder(holder: EquipmentAdapter.Myholder, position: Int) {
        try {

            holder.text.text= list!![position].FAILURE_NICK

            holder.checkbox.setChecked(list[position].isSelected)
            if ( CreateBy1.lists.contains(list!![position].FAILURE_NICK)) {
