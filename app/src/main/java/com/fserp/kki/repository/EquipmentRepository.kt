...
class EquipmentRepository {
    fun equipment(comcode: String): MutableLiveData<List<AllData>>
    {
        val mutableLiveData: MutableLiveData<List<AllData>> = MutableLiveData()
        val praments= Praments()
        praments.PCOMPCODE=comcode
        praments.EQUIPMENTTYPE_NAME=""

        APIClients.newsInstance.getEquipment(praments)?.enqueue(object : Callback<Responses?> {
